...
app.get('/player_info/select_player', function(req,res){
  ...
  var query1 = 'select * from football_players;';
  var query2 = 'select * from football_players where id = ' + ident + ';';
  var query3 = 'select count(*) from football_players inner join football_games on football_players.id = ANY(football_games.players) where football_players.id =' + ident+';';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
  ...
  .then(data => {
    console.log("selected player info: ", data[1][0]);
    console.log("number of games played: ", data[2][0].count)
  	res.render('pages/player_info',{
  			my_title: "Player Stats",
  			result_1: data[0],
  			result_2: data[1][0],
